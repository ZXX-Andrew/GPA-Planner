class GradeCalculator:
    @staticmethod
    def calculate_current_grade(components):
        """Calculate the current grade based on completed components."""
        total_weight = 0
        weighted_sum = 0
        
        for component in components:
            if component.is_graded:  # Only consider components that have been graded
    ...
    def calculate_required_score(components, target_grade):
        ...
        remaining_weight = sum(c.weight for c in components if not c.is_graded)
        
        if remaining_weight == 0:
            return None  # No remaining components
        
        required_score = (target_grade - current_grade) / remaining_weight * 100
